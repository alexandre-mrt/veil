#!/bin/bash
# compile-transfer-poseidon2.sh — Compile transfer_poseidon2.circom and run Groth16 trusted setup
# Usage: bash scripts/compile-transfer-poseidon2.sh [--skip-ptau]
#
# EXPERIMENTAL circuit (docs/research/2026-08-18-poseidon2-vs-poseidon.md) — not deployed,
# not wired into the frontend or Move contracts. Same shape as compile.sh; kept separate
# rather than parametrizing the existing scripts, matching this repo's one-script-per-circuit
# convention (compile.sh, compile-withdraw.sh, compile-compliance.sh).
#
# Requires: circom 2.1.x, snarkjs 0.7.x (global or via npx)
# Output artifacts go to circuits/build-transfer-poseidon2/

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
CIRCUITS_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
BUILD_DIR="$CIRCUITS_DIR/build-transfer-poseidon2"
...
echo "=== Veil Transfer-Poseidon2 (experimental) Circuit Compiler ==="
...
echo "veil-transfer-poseidon2-entropy-$(date +%s)" | snarkjs zkey contribute \
  "$BUILD_DIR/${CIRCUIT_NAME}_0000.zkey" \
  "$BUILD_DIR/${CIRCUIT_NAME}_final.zkey" \
  --name="veil-transfer-poseidon2-dev" \
  -v
...
echo "verifying key. For the real transfer circuit, use scripts/compile.sh."
echo ""
