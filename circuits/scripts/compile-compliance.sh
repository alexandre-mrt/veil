#!/bin/bash
# compile-compliance.sh — Compile compliance.circom and run Groth16 trusted setup
# Usage: bash scripts/compile-compliance.sh [--skip-ptau]
#
...
BUILD_DIR="$CIRCUITS_DIR/build-compliance"
...
echo "=== Veil Compliance Circuit Compiler ==="
...
echo "veil-compliance-entropy-$(date +%s)" | snarkjs zkey contribute \
  "$BUILD_DIR/${CIRCUIT_NAME}_0000.zkey" \
  "$BUILD_DIR/${CIRCUIT_NAME}_final.zkey" \
  --name="veil-compliance-dev" \
  -v
