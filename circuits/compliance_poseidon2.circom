...
// EXPERIMENTAL — Poseidon2 Merkle-hashing variant of compliance.circom, built for the
// 2026-08-18 research experiment (docs/research/2026-08-18-poseidon2-vs-poseidon.md).
// Identical to compliance.circom except the credential Merkle-membership hash (C2):
// MerkleProof (circomlib Poseidon(2), sponge) replaced with MerkleProofPoseidon2
// (Poseidon2(2), compression mode). Credential leaf hash (Poseidon(5)), nullifier
// (Poseidon(3)), context binding (Poseidon(3)) are UNCHANGED — this circuit is not
// compatible with the deployed pool's verifying key and is not wired into the
// frontend or Move contracts. It exists to measure the swap in isolation.
//
