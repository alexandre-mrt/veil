...
// EXPERIMENTAL — Poseidon2 Merkle-hashing variant of transfer.circom, built for the
// 2026-08-18 research experiment (docs/research/2026-08-18-poseidon2-vs-poseidon.md).
// Identical to transfer.circom in every respect except the Merkle-membership hash
// (C0): MerkleProof (circomlib Poseidon(2), sponge) is replaced with
// MerkleProofPoseidon2 (Poseidon2(2), compression mode). Commitment (Poseidon(4)),
// nullifier (Poseidon(4)), and txAmountHash (Poseidon(3)) are UNCHANGED — this
// circuit is not compatible with the deployed pool's verifying key and is not wired
// into the frontend or Move contracts. It exists to measure the swap in isolation.
//
