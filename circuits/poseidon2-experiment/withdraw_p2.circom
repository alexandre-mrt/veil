...
// MOVE CONTRACTS. Mechanically derived from ../withdraw.circom by swapping every Poseidon(n)
...
//
// Constraints (10):
//   C1  commitment well-formed       (Poseidon2Hash(4) identity-bound)
...
//   C6  change commitment            (Poseidon2Hash(4) with domain tag 1, remaining balance)
...
template Withdraw() {
    ...
    signal input commitment;      // Poseidon2Hash(1, cumulativeOld, randomnessOld, userSecret)
    signal input withdrawAmount;  // Amount to withdraw (public for token transfer)
    signal input nullifier;       // Poseidon2Hash(7, userSecret, randomnessOld, cumulativeOld)
    signal input recipientHash;   // Poseidon2Hash(8, recipient) -- binds withdrawal to address
    ...
    // Uses same structure as transfer.circom (domain tag 1, Poseidon2Hash(4))
