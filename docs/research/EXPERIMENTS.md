...
**Standing gap, not a queue item:** at least two other PRs from the 2026-07-14 run
(`research/2026-07-14-arkworks-tee`, adding `arkworks-prover-parity` (KEEP, spike) and
`tee-issuance-spike` (PARK)) branched from a different, incompatible base than this file's lineage and
remain open/unmerged. Their ledger rows are not reflected here because their code
(`experiments/arkworks/`, `experiments/tee/`) doesn't exist on this branch — merging that in is a
repo-integration task bigger than one night's budget, not silently droppable either. Flagging for a
future session or a human to reconcile before those tracks' queue items (#7-#10 below) are picked up
again, so nobody re-does or silently loses that work.

---

## 1. `circuit-test-full-proof-ci` — stop tests from silently degrading to simulation
**Hypothesis:** `circuits/package.json`'s `test` script (and any CI that calls it) doesn't compile the
circuit first, so a fresh clone silently runs the hash-only JS simulation instead of the real compiled
circuit — exactly the root cause `fix-stale-transfer-artifacts` (2026-07-15) closed for `transfer.circom`
specifically. `compliance.circom` and `withdraw.circom` share the same two-mode test structure and are
not currently forced into full-proof mode by any committed script.
**Metric:** CI (or a pre-test step) asserts `FULL_PROOF_AVAILABLE === true` before running the suites,
or fails loudly with a clear message instead of silently degrading; all three circuits' full-proof
mode runs green in that CI.
**Not a research question** — same class of bug as #1 above, ranked first because it prevents that
exact class of gap from recurring for the other two circuits, cheaply.
**Context:** `2026-07-15-fix-stale-transfer-artifacts.md` (Open Questions).
