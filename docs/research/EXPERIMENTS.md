...
1. **Poseidon2 production migration.** 2026-08-27 measured a real, reproducible proving-time win
   from swapping to a domain-separated Poseidon2 sponge — `transfer` -5.4%, `withdraw` -13.3%,
   `compliance` a wash pending t=5 parameters (see `2026-08-27-poseidon2-hash-swap.md`,
   `BASELINE.md`'s "Alternative constructions" section). Research-only variant circuits exist
   under `circuits/bench/poseidon2/full/`; shipping it needs: t=5 Poseidon2 parameters for
   `compliance.circom`'s credential-leaf hash (derive or find a published source — don't hand-roll
   without independent verification), a *documented multi-contributor* trusted-setup ceremony for
   the new zkeys (not dev-only), VK updates via `pool.move`'s existing 1-epoch timelock path, and
   migrating `useProofGeneration.ts` + `scripts/src/*` + `circuits/test/*.test.mjs` to the new hash
   (old commitments/nullifiers are not valid under the new hash — needs a migration plan for
   anything already deposited on testnet). Promoted to top of queue: this is the highest-leverage
   number in the queue and the remaining work is well-scoped, not exploratory.

2. **On-chain gas per entry point.** `BASELINE.md`'s one missing axis. Blocked again on
   2026-08-27 for a clearer, more structural reason than 2026-07-22's ambiguous sandbox denial:
   direct JSON-RPC against `fullnode.testnet.sui.io` is denied by the execution sandbox's network
   policy (confirmed via the proxy status endpoint, not a transient tool-approval hiccup), a
   prebuilt `sui` CLI binary isn't reachable (GitHub releases denied, `403`), and the `sui` package
   on crates.io is an unrelated 2022 placeholder, not the real CLI. **New information:**
   `git clone`/`git ls-remote` against `github.com/MystenLabs/sui.git` *does* succeed even though
   the same host's release-download endpoint doesn't — a from-source build of the `sui` CLI is
   technically reachable, but building Mysten's full workspace (validator, Move VM, RocksDB,
   consensus) is a genuinely multi-hour, disk-heavy undertaking. Budget an **entire dedicated
   night** for this specifically — not "spend the first hour" as previously noted, since the
   remaining blocker is now build time/disk, not connectivity.

3. **Poseidon2 constraint-count follow-up.** 2026-08-27's swap *increased* total R1CS constraints
   (~10-12%) even though proving time dropped, because `@taceo/circom-lib`'s external/internal
   matrix-multiplication templates don't fully collapse into linear combinations in circom's
   optimizer — extra "linear constraints," not a fundamental cost of Poseidon2 itself. A
   hand-optimized sponge wrapper could plausibly make the swap a constraint-count win too, not
   just a proving-time one — worth a night once item 1's migration work is scoped, to avoid
   reintroducing an under-constrained signal in a rushed rewrite.
