...
circuit as "6 public inputs / 11 constraints." See the baseline report for the full drift.
...
| suite | result | note |
|---|---|---|
| `circuits` — `transfer.test.mjs` | **27 / 40 pass, 13 fail** | real bug: witness builder predates the Merkle-proof signals; see report |
| `circuits` — `compliance.test.mjs` | 30 / 30 pass | |
| `circuits` — `withdraw.test.mjs` | 35 / 35 pass | |
| `scripts` — `test-converter.ts` | 109 / 109 pass | |
| `contracts` — `sui move test` | `BLOCKED` | no `sui` CLI |
