# Nightly research loop — standing prompt

This is the prompt configured for Veil's nightly cryptography & scalability research
routine (Claude Code on the web → new routine → repo `veil` → model Fable 5, effort
high). It is preserved here so the routine can be reconstructed or edited without
depending on what's pasted into the scheduler UI.
...
## Bootstrap note (2026-07-26)

The first firing of this routine found that `CLAUDE.md`, `.claude/skills/veil-*`, and the entire
`docs/research/` tree (including the `2026-07-14` worked example this prompt references) did not
exist in the repository — this was, in effect, night 1, not a continuation of an established loop.
This file, `LEDGER.md`, and `EXPERIMENTS.md` were created from scratch as part of that first run
instead of being read as pre-existing state. `CLAUDE.md` and the `veil-*` skills were **not**
fabricated; build/test commands were instead derived directly from `README.md`,
`circuits/package.json`, and `scripts/package.json`. A future run should create `CLAUDE.md` and the
skills if they still don't exist, or drop this note once they do.
