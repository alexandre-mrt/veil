...
This is the stored prompt for Veil's nightly research routine (Claude Code on the web,
Fable 5, effort: high). It is checked into the repo so every run — and anyone reading the
history — can see exactly what the loop was asked to do. Do not edit the substance of this
prompt casually; if the process itself needs to change, that's a decision worth a commit
message of its own.
