# Nightly research loop — stored prompt

This is the prompt configured as the recurring "veil nightly research" routine
(model: Fable 5, effort: high). It is stored here so the loop is self-documenting:
anyone reading `docs/research/` can see the exact instructions that produced
`LEDGER.md`, `EXPERIMENTS.md`, and each dated report, without needing access to
the scheduling app.

Do not edit the behavior of the loop by editing this file alone — the routine
config in the app is the source of truth. Update both together if the process
changes.
