...
This is the prompt configured as a recurring scheduled routine (model: Fable 5, effort:
high) against this repository. It is stored here so the loop is reproducible and
auditable, and so future nights can be diffed against what actually ran.

First run: 2026-07-21. On that night this file, `LEDGER.md`, and `EXPERIMENTS.md` did
not exist yet — the routine had been configured to reference them, but the repository
had never been bootstrapped. This file is that bootstrap.
