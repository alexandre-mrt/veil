# Veil nightly cryptography & scalability research loop — prompt

This is the prompt configured for the recurring "veil nightly research" routine
(model: Fable 5, effort: high). Paste the body below (everything after this
line) as the routine's prompt. This file is the canonical copy — if the two
drift, this file wins.

Bootstrapped 2026-07-18: this is the first night the loop ran. `docs/research/`,
`CLAUDE.md`, and the `veil-*` skills referenced below did not exist yet; the
first run created `LEDGER.md`, `EXPERIMENTS.md`, and this file, and used
`README.md` in place of `CLAUDE.md` for build/test commands since no
`CLAUDE.md` exists in this repository. If one gets added later, prefer it.
