# Nightly research loop — prompt

This is the exact prompt pasted into the scheduled routine (app → new routine → repo `veil` →
model Fable 5, effort high). Keep this file in sync with whatever is actually configured in the
app; if you change the routine, change this file in the same PR so the two never drift.
...
## Bootstrap note (2026-07-19)

The first run of this loop found none of the referenced infrastructure in place: no `CLAUDE.md`,
no `.claude/skills/veil-*`, no `docs/research/` at all. That night's PR created this file plus
`LEDGER.md` and `EXPERIMENTS.md`, and settled queue item #1 (`BASELINE.md`) as far as the sandbox's
network egress policy allowed. See the ledger for what got measured and what got marked BLOCKED.
`CLAUDE.md` and `.claude/skills/` were intentionally left out of that PR — bootstrapping harness
config is a different job than running the research loop, and folding it in would have made the
first night's diff impossible to review as "one experiment."
