...
This is the prompt used by the recurring "veil nightly research" routine
(repo `veil`, model Fable 5, effort high). It lives here so the routine
configuration and the loop's own record of itself stay in sync — if you
change how the loop should behave, edit this file first, then update the
routine's stored prompt to match.
