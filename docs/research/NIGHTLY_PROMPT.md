# Veil nightly cryptography & scalability research loop

This is the prompt fed to the nightly research routine (repo `veil`, model Fable 5, effort
high). It is saved here so the loop is reproducible and auditable — the routine that
created this file is described, verbatim, below.
...
## Bootstrap note (first run, 2026-07-17)

This file, `LEDGER.md`, `EXPERIMENTS.md`, and `.claude/skills/veil-*` did not exist before tonight —
this is the loop's first iteration. There was no `CLAUDE.md` either; the repo's `README.md` and
`docs/SPEC.md` cover the same ground (architecture, build/test commands, deployed addresses) and were
used in its place. No worked-example report existed, so the shape of
`2026-07-17-groth16-baseline.md` sets the precedent future nights should follow. Subsequent runs
should find all of the above in place.
