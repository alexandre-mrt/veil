# Veil nightly cryptography & scalability research loop — routine prompt

This is the prompt to paste into a new scheduled routine (app: new routine → repo `veil` →
model Fable 5 → effort high) to run Veil's nightly research loop. It is saved here so the
routine can be (re)created without hunting through chat history.
