...
This is the prompt configured as a recurring routine (Fable 5, effort: high) that drives Veil's
nightly cryptography & scalability research loop. It lives here so the loop's own instructions are
versioned alongside the ledger and queue they produce, and so the wording can be revised in a PR
like anything else.

This file did not exist before the night of 2026-07-22 — the routine had been configured in the
app, but nothing in `docs/research/` had been created yet: no `LEDGER.md`, no `EXPERIMENTS.md`, no
worked example, no `BASELINE.md`. The first run bootstrapped all of it. If you're reading this as
a fresh instance of the loop and something referenced below still doesn't exist, the same applies:
create it, note that you did, and move on — don't block a whole night on scaffolding.
