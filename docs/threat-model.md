...
| D3 | Spam pool with fake commitments to exhaust dynamic fields | `deposit_and_register` requires standard deposit (100/500/1000 TOKEN per commitment). Minimum cost of 100 TOKEN per griefing attempt; gas itself is negligible (~0.0018 SUI/call measured 2026-09-18 — see `docs/research/BASELINE.md`), so the token minimum, not gas, is what bounds this attack. | Mitigated |
