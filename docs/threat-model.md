...
| E7 | Front-run ZK withdrawal to steal funds | **NOT mitigated — see RR10.** The circuit does compute `recipientHash = Poseidon(8, recipient)` as a public input, but `pool::zk_withdraw` (`contracts/sources/pool.move:571-630`) never extracts or checks it: it reads `commitment` (bytes 0-32), `withdrawAmount` (32-64), `nullifier` (64-96), then skips straight to `newCommitment` (128-160). Bytes 96-128 (`recipientHash`) are never read. `recipient` is a separate, unchecked function argument, so **any caller can resubmit someone else's valid `(proof_bytes, public_inputs_bytes)` with their own address as `recipient`** — the proof still verifies (verification only checks proof-vs-public-inputs, not proof-vs-`recipient`), the nullifier is consumed, and the funds go to the attacker. Confirmed by direct code read against current `main` (not inherited from a stale claim); no existing Move test exercises a recipient/proof mismatch. | **Not mitigated** |
...
| RR10 | `zk_withdraw` recipient not bound to proof (see E7) | **Critical** | `pool::zk_withdraw` never checks `recipientHash` (public input bytes 96-128) against the caller-supplied `recipient` address — it is skipped entirely between extracting `nullifier` and `newCommitment`. Any observer of a pending withdrawal's `(proof_bytes, public_inputs_bytes)` can call `zk_withdraw` themselves with their own `recipient`, redirecting the payout. Deployed to testnet with a valueless faucet token, but the bug is real and unpatched. | **Unfixed.** Needs: an on-chain binding of `recipient` to `recipientHash` (either compute `Poseidon(8, recipient)` on-chain and assert equality, or — simpler, since recipient is already not anonymous, see README "Recipient anonymity: No" — change the circuit's public-input layout to expose `recipient` directly instead of a hash of it, dropping the Poseidon call entirely), a soundness argument, and a negative test proving a substituted `recipient` is rejected. Requires a `withdraw.circom` change and VK rotation either way. |
