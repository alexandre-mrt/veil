...
| RR10 | `zk_withdraw` recipient not bound on-chain | **Critical** | `zk_withdraw` (`contracts/sources/pool.move:571-632`) extracts `recipientHash` from the proof's public inputs (bytes 96-128) but never checks it against the `recipient: address` parameter the caller supplies — despite a code comment on that line claiming "front-running is prevented." A `(proof_bytes, public_inputs_bytes)` pair produced for a withdrawal to address A verifies identically if resubmitted with `recipient = B`: a front-runner or a relayer can copy a pending withdrawal's proof verbatim, substitute their own address, and receive the payout while the original submitter's nullifier is spent. Real fund theft, not a privacy nuance. Found and documented 2026-09-11 (`docs/research/2026-09-11-onchain-gas-baseline.md`, PR #59 — unmerged as of 2026-09-12); confirmed independently against current `main` while auditing the research-loop backlog the same night. **Not fixed as of 2026-09-12.** | **Open — needs an on-chain equality check between `recipientHash` and a field-element encoding of `recipient`, a decided address→field-element convention, a soundness argument, and a negative test.** Pre-mainnet blocker; arguably pre-*any further testnet use* blocker given real funds move through `zk_withdraw` today. |
