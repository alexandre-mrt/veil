...
| D3 | Spam pool with fake commitments to exhaust dynamic fields | `deposit_and_register` requires standard deposit (100/500/1000 TOKEN per commitment). Minimum cost of 100 TOKEN per griefing attempt — recoverable, since the TOKEN lands in the attacker's own commitment. The actual unrecoverable cost floor is SUI gas: 1,832,200 MIST (~0.0018 SUI) net per `deposit_and_register` call, measured against a real local Sui network (`docs/research/2026-09-02-onchain-gas.md`). | Mitigated |
