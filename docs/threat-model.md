...
| RR10 | `zk_withdraw` doesn't bind `recipient` to the proof | Critical | `pool.move::zk_withdraw` never checks the on-chain `recipient: address` argument against the proof's `recipientHash` public input (bytes 96-128 of `public_inputs_bytes` are never extracted or compared, despite a comment claiming otherwise). A valid `(proof_bytes, public_inputs_bytes)` pair verifies identically no matter what `recipient` a caller passes, so a front-runner or a relayer can resubmit a pending withdrawal with their own address and receive the payout; the true owner's nullifier is spent, and their transaction reverts. Direct fund theft, found 2026-09-11 while building `docs/research/2026-09-11-onchain-gas-baseline.md`'s gas harness. | Not fixed. Needs a decided Sui-address-to-BN254-field-element convention (none exists in this repo) plus an on-chain `assert!(Poseidon(8, recipient) == recipientHash)` check, a soundness argument, and a negative test. Tracked as `docs/research/EXPERIMENTS.md` item 1. |
