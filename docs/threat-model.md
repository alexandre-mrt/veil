...
| E7 | Front-run ZK withdrawal to steal funds | Withdrawal circuit computes `recipientHash = Poseidon(8, recipient)` as a public input, but `pool::zk_withdraw` (`contracts/sources/pool.move`) never recomputes or compares it against the `recipient: address` argument actually paid out to — see **RR10**. | **NOT mitigated** — see RR10 |
...
| RR10 | `zk_withdraw` does not check `recipient` against the proof's `recipientHash` | **Critical** | `pool::zk_withdraw` (`contracts/sources/pool.move:571-632`) extracts `recipientHash` (bytes 96-128 of the public inputs) but never recomputes `Poseidon(8, recipient)` on-chain and compares it — it sends `withdraw_amount` straight to whatever `recipient: address` argument the caller passes, independent of the proof. A valid `(proof_bytes, public_inputs_bytes)` pair generated for address A verifies identically when resubmitted with `recipient = B`: a relayer, a malicious mempool observer, or anyone who can front-run/copy a pending withdrawal transaction can redirect the payout to their own address while A's nullifier gets spent, at A's cost. The code comment above this block ("Front-running is prevented: changing recipient invalidates the Groth16 proof") is incorrect — changing the `recipient` *argument* does not change the proof or its public inputs at all; the two are unconnected. No test in `contracts/tests/` exercises this (`grep recipient contracts/tests/*.move` — zero hits). Found and documented 2026-09-11 (unmerged PR #59); re-confirmed independently against current `main` 2026-09-15. | **Unfixed.** Needs: an on-chain equality check (`assert!(hash_address(recipient) == recipient_hash_bytes, E_...)` — requires deciding how a Sui `address` maps to the field element the circuit hashes, matching `withdraw.circom`'s `recipient` private-input encoding), a soundness argument for that mapping, and a negative test proving a mismatched-recipient resubmission is rejected. Promoted to top of `EXPERIMENTS.md`. |
