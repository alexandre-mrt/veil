/**
 * witnesses-poseidon2-merkle.mjs — Valid-witness builders for the two experimental
 * circuits in circuits/experiments/ (transfer_poseidon2_merkle.circom,
 * compliance_poseidon2_merkle.circom).
 *
 * Identical to buildTransferWitness()/buildComplianceWitness() in
 * scripts/bench/witnesses.mjs (same values, same domain tags) EXCEPT the Merkle-path
 * folding uses the Poseidon2 compress function (scripts/bench/poseidon2.mjs) instead of
 * circomlib's Poseidon(2), matching the hasher swap in
 * templates/merkle_proof_poseidon2.circom. All non-Merkle hashing (commitments,
 * nullifiers, tx amount hash, credential leaf, context binding) is untouched and still
 * uses the circomlibjs Poseidon passed in, exactly like witnesses.mjs — this experiment
 * only swaps the Merkle-tree node hasher, not the whole circuit's hash function.
 */
...
function merkleRootFromPathPoseidon2(compress, leaf, pathElements, pathIndices) {
  let node = leaf;
...
export function buildTransferWitness(poseidon, compress) {
  ...
  const merkleRoot = merkleRootFromPathPoseidon2(compress, oldCommitment, pathElements, pathIndices);
  return {
...
export function buildComplianceWitness(poseidon, compress) {
...
export const WITNESS_BUILDERS = {
  transfer_poseidon2_merkle: buildTransferWitness,
  compliance_poseidon2_merkle: buildComplianceWitness,
};
